	...
	SubShader
	{
		...
		#pragma shader_feature _ _ALPHAPREMULTIPLY_ON
			...
			struct Varyings
			{
				...
				float2 pack3 : TEXCOORD6; /* pack3.x = fogFactor  pack3.y = rim */
				UNITY_VERTEX_INPUT_INSTANCE_ID
			...
			half4 Fragment(Varyings input) : SV_Target
			{
				...
				color.rgb = ( color.rgb * 1.1 * input.vertexColor.rgb );
		...
		HLSLINCLUDE
			...
			struct Varyings
			{
				...
				float pack2 : TEXCOORD3; /* pack2.x = rim */
...
/* TCP_DATA u config(unity:"2020.3.2f1";ver:"2.7.0";tmplt:"SG2_Template_URP";features:list["UNITY_5_4","UNITY_5_5","UNITY_5_6","UNITY_2017_1","UNITY_2018_1","UNITY_2018_2","UNITY_2018_3","UNITY_2019_1","UNITY_2019_2","UNITY_2019_3","SHADOW_HSV_MASK","SPECULAR_NO_ATTEN","SPECULAR_SHADER_FEATURE","RIM_VERTEX","RIM_DIR_PERSP_CORRECTION","RIM_LIGHTMASK","RIM_SHADER_FEATURE","SUBSURFACE_AMB_COLOR","SS_MULTIPLICATIVE","SS_NO_LIGHTCOLOR","SS_SHADER_FEATURE","VERTEX_DISP_SHADER_FEATURE","ALBEDO_HSV_MASK","TT_SHADER_FEATURE","DIFFUSE_TINT_MASK","SKETCH_AMBIENT","SKETCH_SHADER_FEATURE","DISSOLVE_CLIP","DISSOLVE_GRADIENT","DISSOLVE_SHADER_FEATURE","SS_SCREEN_INFLUENCE","SILHOUETTE_STENCIL","SILHOUETTE_URP_FEATURE","FOG","DISABLE_BATCHING","ENABLE_META_PASS","WRAPPED_LIGHTING_CUSTOM","ALBEDO_HSV","BUMP_SCALE","PARALLAX","BUMP_SHADER_FEATURE","VERTICAL_FOG_ALPHA","VERTICAL_FOG_COLOR","ENABLE_LIGHTMAP","SSAO","RIM","RIM_DIR","WRAPPED_LIGHTING_OTHER_LIGHTS","RAMP_MAIN_OTHER","RAMP_SEPARATED","CLAMP_LIGHTS_INTENSITY","TEXBLEND_NORMALIZE","BLEND_TEX1","AUTO_TRANSPARENT_BLENDING","NO_AMBIENT","CUBE_AMBIENT","DIRAMBIENT","TEMPLATE_LWRP"];flags:list[];flags_extra:dict[];keywords:dict[RENDER_TYPE="Opaque",RampTextureDrawer="[TCP2Gradient]",RampTextureLabel="Ramp Texture",SHADER_TARGET="3.0",RIM_LABEL="Rim Lighting",BLEND_TEX1_CHNL="r",BLEND_TEX2_CHNL="g",BLEND_TEX3_CHNL="b",BLEND_TEX4_CHNL="a"];shaderProperties:list[,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,sp(name:"Final Color";imps:list[imp_hook(guid:"5b4ba6ee-7201-4353-8c0f-e79084e144db";op:Multiply;lbl:"color.rgb";gpu_inst:False;locked:False;impl_index:0),imp_constant_float(fprc:float;fv:1.1;guid:"7c21450a-9d63-42b3-8ec3-9a6096ed94cc";op:Multiply;lbl:"Final Color";gpu_inst:False;locked:False;impl_index:-1),imp_vcolors(cc:3;chan:"RGB";guid:"f7f81dc1-f9ab-4971-bc6a-2434a0e63456";op:Multiply;lbl:"Final Color";gpu_inst:False;locked:False;impl_index:-1)];layers:list[];unlocked:list[];clones:dict[];isClone:False),sp(name:"Depth Test";imps:list[imp_enum(value_type:0;value:7;enum_type:"ToonyColorsPro.ShaderGenerator.CompareFunction";guid:"34c7b64f-bebe-402b-9855-0c2a191f7b99";op:Multiply;lbl:"Depth Test";gpu_inst:False;locked:False;impl_index:0)];layers:list[];unlocked:list[];clones:dict[];isClone:False),sp(name:"Depth Write";imps:list[imp_enum(value_type:0;value:1;enum_type:"ToonyColorsPro.ShaderGenerator.DepthWrite";guid:"4a083c64-83ee-4f1b-abb9-76575e05ebe7";op:Multiply;lbl:"Depth Write";gpu_inst:False;locked:False;impl_index:0)];layers:list[];unlocked:list[];clones:dict[];isClone:False)];customTextures:list[];codeInjection:codeInjection(injectedFiles:list[];mark:False);matLayers:list[]) */
/* TCP_HASH 0af7a7536faf090ac36993a7e0335869 */
